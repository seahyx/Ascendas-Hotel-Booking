...
import { addBooking,getBookings,getLatestBooking,getBookingWithId } from '../../../server/api/bookingfunctions';
...
app.get('/api/getlatestbooking/:uid', async (req: express.Request, res: express.Response) => {
  ...
  try {
    const latestBooking = await getLatestBooking(parseInt(uid));
    res.status(200).json(latestBooking);
    console.log("getLatestBooking",200);
  } catch (error) {
    res.status(500).json({ error: 'An error occurred while fetching the latest booking.' });
...
app.get('/api/booking/:id', async (req: express.Request, res: express.Response) => {
